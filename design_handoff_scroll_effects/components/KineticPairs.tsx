...
/**
 * Pinned "Power of AND" sequence — GSAP ScrollTrigger + Tailwind.
 *
 ...
 */
export default function KineticPairs({ pairs }: { pairs: Pair[] }) {
  ...
  useGSAP(
    () => {
      ...
      mm.add('(min-width: 768px) and (prefers-reduced-motion: no-preference)', () => {
        ...
        return () => { delete root.current!.dataset.animated; };
      });
  ...
  return (
    <section
      ref={root}
      className="group/k relative bg-navy py-24 data-[animated=true]:h-screen data-[animated=true]:py-0"
    >
      <div className="flex flex-col items-center justify-center gap-[10vh] px-6 group-data-[animated=true]/k:h-screen group-data-[animated=true]/k:gap-0 group-data-[animated=true]/k:overflow-hidden">
        {pairs.map(([a, b], i) => (
          <div
            key={i}
            data-pair
            className="relative flex items-center justify-center gap-[0.18em] whitespace-nowrap font-display text-[clamp(34px,8vw,128px)] font-extrabold uppercase tracking-[-0.03em] group-data-[animated=true]/k:absolute group-data-[animated=true]/k:inset-0"
          >
            <span data-kw="l" className="inline-block">{a}</span>
            <span className="font-normal text-acid">&amp;</span>
            <span data-kw="r" className="inline-block">{b}</span>
          </div>
        ))}
      </div>
    </section>
